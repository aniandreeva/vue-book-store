...
<script>
    ...
    export default {
        ...
        methods: {
            handleSubmit() {
                axios.get(`http://localhost:3000/student-books?title_like=` + this.searchTerm)
                    .then(response => {
        ...
        created() {
            axios.get(`http://localhost:3000/student-books`)
                .then(response => {
                    this.books = response.data;
                });
        }
